...
public class EventHandler extends BaseHandler {
    ...
    public Document onProcess() {
        if (MsgContentConstant.EVENT_SCAN.equals(eventName)) {
            ...
            // exchange the user when response,TO to FROM,FROM to TO
            BaseMsgEvent baseMsgEvent = new BaseMsgEvent(msgIn.getToUserName(), msgIn.getFromUserName());
            MsgResponser baseResponser = new MsgResponser(baseMsgEvent);
            return baseResponser.createText(TipConstant.TIP_SUBSCRIBE);
        }
        return null;
    }
 ...
 * baseMsgEvent.setToUserName(msgIn.getFromUserName()); MsgResponser
 * baseResponser = new MsgResponser(baseMsgEvent); PoJoArticle[] articles = new
 * PoJoArticle[2]; PoJoArticle article1 = new PoJoArticle();
 * article1.setDescription("desc1"); article1.setDescription("description1");
 * article1.setPicUrl("picUrl1"); article1.setTitle("title1");
 * article1.setUrl("url1"); PoJoArticle article2 = new PoJoArticle();
 * article2.setDescription("desc2"); article2.setDescription("description2");
 * article2.setPicUrl("picUrl2"); article2.setTitle("title2");
 * article2.setUrl("url2"); articles[0]=article1; articles[1]=article2;
 */
